...
class CreateTranspileTestCase(unittest.TestCase):
    @classmethod
    def setUpClass(self):
        ...
        self.input_path = "transpiler/data/transpiler_input_test.qasm"
        upload_response = self.rest_client.post_request_file(self.input_path, self.upload_api)
        self.test_file_path = json.loads(upload_response.content)["fileName"]
    ...
    def setUp(self):
        ...
        self.test_request_body["file_path"] = self.test_file_path

    def tearDown(self):
        self.logger.info("Test %s Finished" % (self._testMethodName))

    def test_post_transpile_success_response(self):
    ...
    def test_post_transpile_invalid_body(self):
        test_request = self.test_request_body

        # Remove required parameter
        test_request.pop("file_path")
        response = self.rest_client.post_request(
    ...
    def test_get_transpile_unknown_request_404(self):
    ...
    def test_transpile_invalid_file_path(self):
        test_request = self.test_request_body

        test_request["file_path"] = "invalid_path"
        # Post request
    ...
    def test_end_to_end_transpile(self):
        # Post request
        post_transpile_response = self.rest_client.post_request(
            self.test_request_body, self.api, self.test_header
        )
        expected_total_num_operations = 35

        request_id = json.loads(post_transpile_response.content)["request_id"]
        retries = 0
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(result["request_id"], request_id)
        self.assertEqual(result["total_num_operations"], expected_total_num_operations)

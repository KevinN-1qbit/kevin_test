...
class Parse:
    ...
    def _get_gate_list_after_approx(gate_list, rz_approx, gate_index_lookup):
        ...
        for gate_index in sorted(gate_index_lookup.keys(), reverse=True):
            ...
            for g in rz_approx[gate_index_lookup[gate_index][0]][::-1]:
                ...
                gate_approx_created.append(
                    (g, gate_index_lookup[gate_index][1]))
    ...
    def perform_rz_decomposition(self, gate_list):
        ...
        gate_index_lookup, unique_rz_gates = \
            self._rz_gates_index_in_original_list(gate_list)
...
class ParseQasm(Parse):
    ...
    def entire_list(self) -> list:
        ...
        for line in qasm_list:
            ...
            if line[0] == "include":  # if line is a header
                ...
                lib_path = f"{path_to_root_dir}/{lib_name}"
    ...
    def store_gate_info(data, gate_info_dict):
        ...
        try:
            gate_name = data[0][0: data[0].index("(")]
        except BaseException as e:
            # todo: except what? too broad catch case. add exception
            gate_name = data[0]
        ...
        try:
            assigned_parameters = (data[0][data[0].index("(") + 1 :
                                           data[0].index(")")]
                                   ).split(",")
        except BaseException as e:
            # todo Catch the intended exception type here, only
        ...
        assigned_qubits = (data[1].replace(";", "")).split(",")
    ...
    def breakdown_3qubit_gate(
            self, gate_info_dict, gate, saved_values,
            gate_name, num_qubits_per_gate, qubit_name) -> None:
        ...
        if definition == []:
            raise Exception("3-qubit gate must be decomposed "
                            "into 1 or 2 qubit gates")
            ...
            if counter % 2 == 0:
                ...
                if gate_info[1] == 3:
                    ...
                    self.breakdown_3qubit_gate(
                        gate_info_dict, cur_gate, saved_values,
                    ...
                    try:
                        ...
                        expression_list = (inst_name[inst_name.index("(") + 1:
                                                     inst_name.rindex(")")]
                                           ).split(",")
                        ...
                        final_name = cur_gate + "("
                        for count, param in \
                                enumerate(simplified_expression_list):
        ...
        for line in raw_data:
            ...
            # and (inside_gate_bracket_flag == 0):
                ...
                if gate_description[1] == 3:
                    ...
                    self.breakdown_3qubit_gate(
                        gate_info_dict, gate, saved_values, gate_name,
                        num_qubits_per_gate, qubit_name)
                    self.gate_count += 1
                    continue
                ...
                qubit_names_str = line[1].strip(";")
                qubit_names_list = [word.strip()
                                    for word in qubit_names_str.split(",")]
    ...
    def encode_qubit_name(qubit_names):
        ...
        qubit_IDs = [[int(re.findall(r"\d+", q)[0]) for q in name]
        ...
        map_to_fix_qubit_numbering = \
            dict((v, i) for i, v in enumerate(unique_qubits))

        # renumber the qubits
        encoded = [[map_to_fix_qubit_numbering[id_] for id_ in ids]
                   for ids in qubit_IDs]
        ...
        with open(file_path, "r") as f:
            for line in f:
                ...
                if raw != ["\n"]:
                    ...
                    qubit_int = [int(ele)
                                 for ele in re.findall(r"\b\d+\b", qubit_str)]
            ...
            if gate == "allocate":
                ...
                for q in qubit:
                    if q in measured_qubit:
                        ...
                        # qubit IDs
                        raise ValueError("Measured qubit being used again. "
                                         "The qubit in question is ", q)

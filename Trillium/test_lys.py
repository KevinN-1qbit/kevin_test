...
def get_expected_sys_argv(
        input_file_path: str, language: _Language) -> list[str]:
    return f"{__file__} -input {input_file_path} -language {language}".split()


def get_expected_cfg_dict_from_parsing_sys_argv(
        input_file_path: str, language: _Language) -> dict[str, Any]:
    cfg = {"input_file": input_file_path,
           "language": language,
           "combine": True,
           "recompile_cpp": False,
           "epsilon": 1e-10}
...
def test_main(input_file: str, language: _Language, monkeypatch) -> None:
    ...
    expected_cfg = \
        get_expected_cfg_dict_from_parsing_sys_argv(input_file_path, language)

...
class NoiseParams:
    ...
    def transmon(params) -> "NoiseParams":
        ...
        ps["idle_R"] = NoiseParams.p_idling(
            tP, T1, T2
        )  # R gate is used to initialize/prep qubits
        ...
        ps["MR"] = ps["M"]  # assumes in-software perfect reset
...
class NoiseModel:
    ...
    def transmon(ps) -> "NoiseModel":
        return NoiseModel(
            ...
            noisy_gates={
                ...
                "R": ps[
                    "P"
                ],  # this is more accurate than ps["R"] since R gate is used to initialize/prep qubits
                "M": ps["M"],
                "MR": ps["M"],
                "P": ps["P"],
            },
    ...
    def noisy_circuit(self, circuit: stim.Circuit) -> stim.Circuit:
        ...
        def flush():
            ...
            idle_qubits = sorted(qs - active_qubits_1)
            if active_qubits_1 and idle_qubits and self.idle_1 > 0:
                current_moment_post.append_operation(
                    "DEPOLARIZE1", idle_qubits, self.idle_1
                )

            idle_qubits = sorted(qs - active_qubits_2)
            if active_qubits_2 and idle_qubits and self.idle_2 > 0:
                current_moment_post.append_operation(
                    "DEPOLARIZE1", idle_qubits, self.idle_2
                )

            idle_qubits = sorted(qs - measured_qubits)
            if measured_qubits and idle_qubits and self.measure_idle > 0:
                current_moment_post.append_operation(
                    "DEPOLARIZE1", idle_qubits, self.measure_idle
                )

            idle_qubits = sorted(qs - prep_qubits)
            if prep_qubits and idle_qubits and self.prep_idle > 0:
                current_moment_post.append_operation(
                    "DEPOLARIZE1", idle_qubits, self.prep_idle
                )
            ...
            active_qubits_1.clear()
            active_qubits_2.clear()
        ...
        for op in circuit:
            if isinstance(op, stim.CircuitRepeatBlock):
                ...
                # )
                if op.name in ANY_CLIFFORD_1_OPS:
                    active_qubits_1 |= touched_qubits

                if op.name in ANY_CLIFFORD_2_OPS:
                    active_qubits_2 |= touched_qubits

                if op.name in MEASURE_OPS:
                    measured_qubits |= touched_qubits

                if op.name in RESET_OPS:
                    prep_qubits |= touched_qubits
            else:
                raise NotImplementedError(repr(op))
        flush()

...
py::tuple run_lys_with_mea(vector<Rotation> rotations, vector<int> roIndex, vector<Measure> measures, vector<int> meIndex, bool removeNonT, int time_out)
{
    ...
    pair<vector<vector<std::shared_ptr<Operation>>>, int> compilerResults_layered = compiler.runLysCompiler(removeNonT, false, time_out);
...
py::tuple run_lys_default_mea(int numDefaultMeasurements, vector<Rotation> rotations, bool removeNonT, int time_out)
{
    ...
    pair<vector<vector<std::shared_ptr<Operation>>>, int> compilerResults_layered = compiler.runLysCompiler(removeNonT, false, time_out); // if layer==false, this will return an empty list
    int startingIdxToRemoveAfterMeasure = compilerResults_layered.second;                                                                 // if layer==false, we only care about startingIdxToRemoveAfterMeasure, the "compilerResults_layered" itself will be empty
    vector<shared_ptr<Operation>> compiledVec;                                                                                            // vector that will hold the compiled circuit output
    compiledVec.insert(compiledVec.end(), compiler.circuit.begin(), compiler.circuit.begin() + startingIdxToRemoveAfterMeasure);
...
py::tuple run_lys_section(vector<vector<Rotation>> rotVecVec, vector<vector<int>> rotInd, vector<vector<Measure>> meaVecVec, vector<vector<int>> meaInd, py::list ancilla, bool removeNonT, int time_out)
{
    ...
    for (int i = 0; i < gateVecVec.size(); i++)
    {
        ...
        pair<vector<vector<std::shared_ptr<Operation>>>, int> compilerResults = compiler.runLysCompiler(removeNonT, false, time_out); // since layer=false, we only care about the int startingIdxToRemoveAfterMeasure; the vecvec will be empty

...
public:
    ...
    //  * It will :
    //  *      1. Try to combine adjacent gates
    //  *      2. Push T forward
    //  *      3. Create layers for the T gates
    //  *      4. Try to combine gates into each layer
    //  *      5. Do 1-3 until there is no more change
    //  *
     ...
     *
     */
    bool implementNoOrderingRotationLayerCombination(std::vector<std::shared_ptr<Operation>> &layer1, std::vector<std::shared_ptr<Operation>> &layer2);

    /**
     *
     */
    bool noOrderingRotationLayerCombination(std::vector<std::shared_ptr<Operation>> &layer1, std::vector<std::shared_ptr<Operation>> &layer2);
     ...
     *
     * @param removeNonT   bool : Combine Clifford+Pauli to Measure.
    ...
    std::pair<std::vector<std::vector<std::shared_ptr<Operation>>>, int> runLysCompiler(bool removeNonT, bool layer, int time_out);
};
